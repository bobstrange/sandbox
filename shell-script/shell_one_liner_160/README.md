...
ブレース展開で解決できる => これはできた
3 単語で無い FQDN の作り方 => ブレースの中で何も文字列を書かずにからの項目のみを記載する

```bash
echo {A,B,C,}.{D,E,F}.{jp,com} | xargs -n1
```

`xargs -n1` でスペースを改行に変えているが、`tr` の方が直感的

```bash
echo {A,B,C,}.{D,E,F}.{jp,com} | tr ' ' '\n'
```

ランダムにするためには `sort -R` が使える
...
shellscript 内で作成するリソースを `EXIT` シグナルを受け取った時に消すようにしておくと、
途中で終了するときに後始末ができる

```bash
trap 'rm xxx yyy zzz' EXIT
```
